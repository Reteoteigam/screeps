  ...
  run: function(creep) {
    ...
    if (!creep.memory.filling || creep.store.getUsedCapacity() >= 0.7) {
      ...
      if (!target || target.store.getFreeCapacity(RESOURCE_ENERGY) == 0) {
        ...
        targets.sort((a, b) => (a.store.getUsedCapacity(RESOURCE_ENERGY) - 49) - (b.store.getUsedCapacity(RESOURCE_ENERGY) - 49) + ((a.pos.getRangeTo(creep.pos) - b.pos.getRangeTo(creep.pos)) * 3));
        if (targets.length >= 1) {
